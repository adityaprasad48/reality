import React from "react";

const FocusElement = () => {
  return (
    <div class="KOW1OA" style="transform: translate(693.427px, 104.754px);">
      <div
        class="OAPPpQ Jvdx_Q FB7MHA"
        style="width: 335.868px; height: 359.899px; transform: translate(-925.553px, -337.134px) rotate(0deg);"
      >
        <div className="USePRA wqPZwA"></div>
        <div className="wH0DGA" tabIndex="-1"></div>
      </div>
      <div
        class="OAPPpQ FB7MHA"
        style="width: 335.868px; height: 359.899px; transform: translate(-925.553px, -337.134px) rotate(0deg);"
      >
        <div className="o0HGhg YE6wQA" tabIndex="-1">
          <div className="kLSz8Q">
            <div className="kos7Ag OElbmQ uV9new OkgeDg cG_uWg">
              <div
                class="z9y2Mg wJ9_lg"
                style="clip-path: inset(0px 8px);"
              ></div>
              <div className="GgAMiQ"></div>
            </div>
            <div className="kos7Ag OElbmQ _2Ku7hw pzagrA Q4Ofvg">
              <div
                class="z9y2Mg wJ9_lg"
                style="clip-path: inset(8px 0px);"
              ></div>
              <div className="GgAMiQ"></div>
            </div>
            <div className="kos7Ag OElbmQ _2Ku7hw WSOU1g Q4Ofvg">
              <div
                class="z9y2Mg wJ9_lg"
                style="clip-path: inset(8px 0px);"
              ></div>
              <div className="GgAMiQ"></div>
            </div>
            <div className="kos7Ag OElbmQ uV9new yer96g cG_uWg">
              <div
                class="z9y2Mg wJ9_lg"
                style="clip-path: inset(0px 8px);"
              ></div>
              <div className="GgAMiQ"></div>
            </div>
            <div className="kos7Ag LUSARA LYCvQw">
              <div className="z9y2Mg wJ9_lg"></div>
              <div className="GgAMiQ"></div>
            </div>
            <div className="kos7Ag r6m0yg LYCvQw">
              <div className="z9y2Mg wJ9_lg"></div>
              <div className="GgAMiQ"></div>
            </div>
            <div className="kos7Ag wAGCuQ LYCvQw">
              <div className="z9y2Mg wJ9_lg"></div>
              <div className="GgAMiQ"></div>
            </div>
            <div className="kos7Ag xWYclg LYCvQw">
              <div className="z9y2Mg wJ9_lg"></div>
              <div className="GgAMiQ"></div>
            </div>
            <div class="wM7XEg q_0Aqg" style="--floating-offset: 32px;">
              <div className="kos7Ag VvGHxQ VvGHxQ SXvZVg">
                <div className="z9y2Mg wJ9_lg"></div>
                <span className="GgAMiQ">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                  >
                    <path
                      fill="currentColor"
                      d="M15.25 18.48V15a.75.75 0 1 0-1.5 0v4c0 .97.78 1.75 1.75 1.75h4a.75.75 0 1 0 0-1.5h-2.6a8.75 8.75 0 0 0-2.07-15.53.75.75 0 1 0-.49 1.42 7.25 7.25 0 0 1 .91 13.34zM8.75 5.52V9a.75.75 0 0 0 1.5 0V5c0-.97-.78-1.75-1.75-1.75h-4a.75.75 0 0 0 0 1.5h2.6a8.75 8.75 0 0 0 2.18 15.57.75.75 0 0 0 .47-1.43 7.25 7.25 0 0 1-1-13.37z"
                    ></path>
                  </svg>
                </span>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="E_6_9g"></div>
      <div
        class="S_7cdA VUbRqg"
        style="transform: translate(-828.619px, -407.134px); pointer-events: auto;"
      >
        <div className="YcMrMg nGtgNA">
          <div className="_5V7OnQ">
            <div className="_34nGcQ">
              <div className="">
                <div className="uLt0uw">
                  <button
                    className="_1QoxDw Qkd66A tYI0Vw o4TrkA Eph8Hg cclg9A YPTJew Qkd66A tYI0Vw HySjhA cwOZMg zQlusQ uRvRjQ pgaA2w NYDPnA"
                    type="button"
                    aria-label="Add comment"
                  >
                    <span className="TcNIhA">
                      <span aria-hidden="true" className="NA_Img dkWypw">
                        <svg
                          width="24"
                          height="24"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M21.747 11.022A7.75 7.75 0 0 1 14.063 19c-.017.007-.044.02-.083.04a4 4 0 0 0-.439.29c-.358.263-.782.624-1.198.996-.414.37-.805.74-1.097 1.018l-.146.14-.206.196a5.317 5.317 0 0 1-.18.16l-.001.001c-.009.007-.055.043-.118.075l-.002.001a.756.756 0 0 1-.875-.137.75.75 0 0 1-.22-.53v-2.266A7.75 7.75 0 0 1 10 3.5h4c.198 0 .394.007.588.022-.232.46-.401.956-.497 1.479L14 5h-4a6.25 6.25 0 1 0 0 12.5h.998v2.02c.112-.102.227-.206.344-.31.428-.384.894-.782 1.31-1.088.207-.153.417-.294.614-.4.166-.09.438-.222.734-.222a6.25 6.25 0 0 0 6.247-6.05 5.461 5.461 0 0 0 1.5-.428z"
                            fill="currentColor"
                          ></path>
                          <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M18.75 8.75a.75.75 0 0 0 1.5 0v-2h2a.75.75 0 0 0 0-1.5h-2v-2a.75.75 0 0 0-1.5 0v2h-2a.75.75 0 0 0 0 1.5h2v2z"
                            fill="currentColor"
                          ></path>
                        </svg>
                      </span>
                    </span>
                  </button>
                </div>
              </div>
            </div>
            <div className="">
              <div className="uLt0uw">
                <button
                  className="_1QoxDw Qkd66A tYI0Vw o4TrkA Eph8Hg cclg9A YPTJew Qkd66A tYI0Vw HySjhA cwOZMg zQlusQ uRvRjQ pgaA2w NYDPnA wcr_Pg"
                  type="button"
                  aria-label="Duplicate"
                >
                  <span className="TcNIhA">
                    <span aria-hidden="true" className="NA_Img dkWypw">
                      <svg
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M5 3h8a2 2 0 0 1 2 2v.5h-1.5V5a.5.5 0 0 0-.5-.5H5a.5.5 0 0 0-.5.5v10a.5.5 0 0 0 .5.5h2.5V17H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2Zm6 5.5a.5.5 0 0 0-.5.5v10a.5.5 0 0 0 .5.5h8a.5.5 0 0 0 .5-.5V9a.5.5 0 0 0-.5-.5h-8ZM19 7h-8a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2Z"
                          fill="currentColor"
                        ></path>
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M15 11a.75.75 0 0 0-.75.75v1.5h-1.5a.75.75 0 0 0 0 1.5h1.5v1.5a.75.75 0 0 0 1.5 0v-1.5h1.5a.75.75 0 0 0 0-1.5h-1.5v-1.5A.75.75 0 0 0 15 11Z"
                          fill="currentColor"
                        ></path>
                      </svg>
                    </span>
                  </span>
                </button>
              </div>
            </div>
            <div className="">
              <div className="uLt0uw">
                <button
                  className="_1QoxDw Qkd66A tYI0Vw o4TrkA Eph8Hg cclg9A YPTJew Qkd66A tYI0Vw HySjhA cwOZMg zQlusQ uRvRjQ pgaA2w NYDPnA tqgL3A"
                  type="button"
                  aria-label="Delete"
                >
                  <span className="TcNIhA">
                    <span aria-hidden="true" className="NA_Img dkWypw">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                      >
                        <path
                          fill="currentColor"
                          d="M8 5a3 3 0 0 1 3-3h2a3 3 0 0 1 3 3h4.25a.75.75 0 1 1 0 1.5H19V18a3 3 0 0 1-3 3H8a3 3 0 0 1-3-3V6.5H3.75a.75.75 0 0 1 0-1.5H8zM6.5 6.5V18c0 .83.67 1.5 1.5 1.5h8c.83 0 1.5-.67 1.5-1.5V6.5h-11zm3-1.5h5c0-.83-.67-1.5-1.5-1.5h-2c-.83 0-1.5.67-1.5 1.5zm-.25 4h1.5v8h-1.5V9zm4 0h1.5v8h-1.5V9z"
                        ></path>
                      </svg>
                    </span>
                  </span>
                </button>
              </div>
            </div>
            <div className="">
              <div className="">
                <div className="uLt0uw">
                  <button
                    className="_1QoxDw Qkd66A tYI0Vw o4TrkA Eph8Hg cclg9A YPTJew Qkd66A tYI0Vw HySjhA cwOZMg zQlusQ uRvRjQ pgaA2w NYDPnA"
                    type="button"
                    aria-label="More"
                    aria-pressed="false"
                  >
                    <span className="TcNIhA">
                      <span aria-hidden="true" className="NA_Img dkWypw">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="24"
                          height="24"
                          viewBox="0 0 24 24"
                        >
                          <path
                            fill="currentColor"
                            fill-rule="evenodd"
                            d="M5 14a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm7 0a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm7 0a2 2 0 1 1 0-4 2 2 0 0 1 0 4z"
                          ></path>
                        </svg>
                      </span>
                    </span>
                  </button>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default FocusElement;
